...
function languageInstruction(lang: 'en' | 'fa'): string {
	if (lang === 'fa') {
		return 'پاسخ را به زبان فارسی و به صورت واضح تولید کن.';
	}
	return 'Respond in clear English.';
}

export function buildTextSummaryPrompt(input: string, detail: DetailLevel, lang: 'en' | 'fa'): string {
...
export function buildCodeExplainPrompt(code: string, detail: DetailLevel, lang: 'en' | 'fa'): string {
	const guidance = detail === 'short'
